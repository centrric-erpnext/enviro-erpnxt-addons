...
def execute():
    ...
    if not frappe.db.exists("Customer", "Test Client"):
        frappe.get_doc({"doctype": "Customer", "customer_name": "Test Client", "customer_group": "All Customer Groups", "territory": "All Territories"}).insert(ignore_permissions=True)
    if not frappe.db.exists("Item", "Enviro Waste Service"):
        frappe.get_doc({"doctype": "Item", "item_code": "Enviro Waste Service", "item_group": "Services"}).insert(ignore_permissions=True)
    ...
    print(f"EXPECTED: Client = Pending, Accounts = (Empty)")
    print(f"ACTUAL:   Client = {quote_a.custom_client_approval_status}, Accounts = {quote_a.custom_accounts_approval_status}")
    ...
    print(f"EXPECTED: Client = Approved, Accounts = Pending")
    print(f"ACTUAL:   Client = {quote_a.custom_client_approval_status}, Accounts = {quote_a.custom_accounts_approval_status}")
    ...
    print(f"EXPECTED: Client = Not Required, Accounts = Pending")
    print(f"ACTUAL:   Client = {quote_b.custom_client_approval_status}, Accounts = {quote_b.custom_accounts_approval_status}")
