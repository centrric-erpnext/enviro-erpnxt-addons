...
def execute():
    ...
    def get_or_create(fieldname, label, fieldtype, options=None, depends_on=None):
        for f in dt.fields:
            if f.fieldname == fieldname:
                ...
                if depends_on is not None: f.depends_on = depends_on
                return f
        
        print(f"Appending new: {fieldname}")
        return dt.append("fields", {
            "fieldname": fieldname,
            "label": label,
            "fieldtype": fieldtype,
            "options": options,
    ...
    # Create the fields
    get_or_create("type_of_reoccurring", "Type of Reoccurring", "Select", options="\nin Weeks Only\nin Daily", depends_on="eval:doc.is_reoccurring_quote=='YES'")
    ...
    new_injections = ["type_of_reoccurring", "col_daily_1", "monday", "tuesday", "wednesday", "thursday", "col_daily_2", "friday", "saturday", "sunday"]
    ...
    for f in dt.fields:
        if f.fieldname not in new_injections:
            ...
            if f.fieldname == "frequency_in_weeks":
                ordered_names.extend(["col_daily_1", "monday", "tuesday", "wednesday", "thursday", "col_daily_2", "friday", "saturday", "sunday"])
