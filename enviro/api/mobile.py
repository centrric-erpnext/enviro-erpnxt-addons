	...
	try:
		...
		jobs = frappe.get_all(
			...
			fields=[
				...
				"vehicle",
				"driver",
				"driver.employee_name as driver_name",
				"custom_waste_type",
			],
		)
		...
		for job in jobs:
			...
			# Determine status string for Flutter based on timestamps
			flutter_status = "Pending"
			if job.get("arrive_facility_time"):
				flutter_status = "arrived_at_enviro_facility"
			elif job.get("depart_depot_time"):
				flutter_status = "departed_waste_depot"
			elif job.get("arrive_depot_time"):
				flutter_status = "arrived_at_waste_depot"
			elif job.get("contact_signature"):
				flutter_status = "completed"
			elif job.get("job_finished_time"):
				flutter_status = "job_finished"
			elif job.get("job_started_time"):
				flutter_status = "job_started"
			elif job.get("depart_facility_time"):
				flutter_status = "departed_enviro_facility"
			...
			formatted_jobs.append(
				{
					...
					"sales_person": job.site_contact_name,
					"vehicle": job.vehicle,
					"start_date": start_dt,
					"end_date": end_dt,
					...
					"drivers": [
						{
							...
							"vehicle_id": 1,
							"dp": "",
							"registration": job.vehicle,
							"preinspectioncheck": bool(
								frappe.db.exists(
									"Vehicle Pre-Inspection Check",
									{
										"vehicle": job.vehicle,
										"driver": job.driver,
										"creation": ["like", f"{nowdate()}%"],
									},
								)
							),
...
def _map_site_to_flutter(site, site_int_id):
	...
	return {
		...
		"industry_type": int(site.get("industry_type"))
		if site.get("industry_type") and str(site.get("industry_type")).isdigit()
		else None,
		"dp_thumbnail": None,
...
def get_site_details(site_id=None):
	try:
		...
		db_site_name = frappe.db.sql(
			"SELECT name FROM `tabSite` WHERE CRC32(name) = %s LIMIT 1", (clean_site_id,)
		)
		if db_site_name:
...
def get_site_previous_sales(site_id=None, page=1, limit=10):
	try:
		...
		jobs = frappe.get_all(
			"Enviro Job",
			filters={"site": actual_site_name, "status": ["in", ["Completed", "Closed"]]},
			fields=[
				"name",
				"customer",
				"custom_waste_type",
				"arrive_waste_depot_time",
...
def get_waste_types_in_site(site_id=None):
	try:
		...
		jobs = frappe.get_all("Enviro Job", filters={"site": actual_site_name}, fields=["custom_waste_type"])
...
def add_vehicle_pre_inspection():
	try:
		...
		frappe.log_error(title="Pre-Inspection DEBUG Params", message=str(params))
...
def get_job_card():
	try:
		...
		result = {
			...
			"created_date_time": str(job.creation)
			if job.get("creation") and str(job.creation).strip()
			else "2024-01-01",
			"date": str(job.scheduled_start_date)
			if job.get("scheduled_start_date") and str(job.scheduled_start_date).strip()
			else "2024-01-01",
		}
		return raw_json(result)
	except Exception:
...
def get_sales_quote_details(quote_id, page=1, limit=10, year=None, month=None):
	try:
		...
		quotes = frappe.get_all(
			"Quotation",
			filters=filters,
			fields=["name", "customer_name", "transaction_date", "status", "grand_total", "creation"],
			start=(page - 1) * limit,
			page_length=limit,
			order_by="creation desc",
		)
			...
			result.append(
				{
					...
					"date": quote.transaction_date.strftime("%Y-%m-%d")
					if hasattr(quote.transaction_date, "strftime")
					else str(quote.transaction_date),
					"status": quote.status,
	...
	try:
		...
		quotes = frappe.get_all(
			...
			fields=[
				...
				"grand_total",
				"creation",
				"custom_client_approval_status",
				"custom_requires_client_approval",
			],
			start=(page - 1) * limit,
			page_length=limit,
			order_by="creation desc",
...
def get_sales_job_list(page=1, limit=10, search=None):
	try:
		...
		jobs = frappe.get_all(
			...
			filters=filters,
			or_filters=or_filters,
			fields=["name", "customer", "site_name", "creation", "status", "custom_waste_type"],
			start=(page - 1) * limit,
			page_length=limit,
			order_by="creation desc",
		)
			...
			result.append(
				{
					...
					"job_code": job.name,
					"client_name": job.customer,
					"date": job.creation.strftime("%Y-%m-%d %H:%M:%S")
					if hasattr(job.creation, "strftime")
					else str(job.creation),
					"status": job.status,
					"job_card_code": job.name,
...
def get_sales_quote_reg_details(id):
	try:
		...
		result = {
			"quote": {"quote_file": "", "received_file": "", "attached_files": [], "template_response": []},
			"schedule": {"team": []},
		}
...
def get_sales_job_details(id):
	try:
		...
		db_job_name = frappe.db.sql(
			"SELECT name FROM `tabEnviro Job` WHERE CRC32(name) = %s LIMIT 1", (str(id),)
		)
		if db_job_name:
		...
		if frappe.db.exists("Enviro Job", actual_job_name):
			...
			for tm in getattr(job, "team_members", []):
				result["schedule"]["team"].append({"id": tm.name, "team_member_name": tm.team_member})
