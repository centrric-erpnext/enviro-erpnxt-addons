...
def execute():
    ...
    workflows = frappe.get_all("Workflow", filters={"document_type": "Quotation"}, fields=["name", "is_active"])
    print("Workflows on Quotation: ", workflows)

    # Let's also check for custom fields on Quotation
    custom_fields = frappe.get_all("Custom Field", filters={"dt": "Quotation"}, fields=["fieldname", "fieldtype", "label"])
