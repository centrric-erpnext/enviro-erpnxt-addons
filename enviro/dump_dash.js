...
function render() {
   ...
   if(unscheduled.length === 0) html += `<tr><td colspan="4" style="text-align:center; padding: 30px; color:#888;">All authorized jobs have been scheduled.</td></tr>`;
   
   unscheduled.forEach(j => {
       html += `
           <tr>
               <td><span class="job-pill" onclick="frappe.set_route('Form', 'Enviro Job Card', '${j.name}')">${j.name}</span></td>
               ...
               <td><button class="btn-schedule" onclick="window.ph2_modal('${j.name}')">Schedule</button></td>
           </tr>
       `;
   });
   ...
   state.dates.forEach(d => {
       ...
       html += `<div class="matrix-row">
           <div class="matrix-row-label">
              <span style="color:#64748b; font-size: 10px;">${d.toLocaleDateString("en-AU", { weekday: 'long'})}</span>
              <span style="color:#0f172a; font-size: 12px;">${frappe.datetime.str_to_user(dStr)}</span>
           </div>
           <div class="matrix-cells-wrapper sync-scroll">
       `;
       
       state.vehicles.forEach(v => {
           let vJobs = state.jobs.filter(j => j.scheduled_start_date === dStr && j.vehicle === v.name);
           
           html += `<div class="matrix-cell">`;
           vJobs.forEach(vj => {
               html += `<div class="sched-event" onclick="frappe.set_route('Form', 'Enviro Job Card', '${vj.name}')">
...
window.ph2_modal = function(job_id) {
    let d = new frappe.ui.Dialog({
        ...
        fields: [
            {
                label: 'Scheduled Start Date', fieldname: 'scheduled_start_date', fieldtype: 'Date', reqd: 1, 
                default: frappe.datetime.get_today()
            },
            {
                label: 'Scheduled Start Time', fieldname: 'scheduled_start_time', fieldtype: 'Time', reqd: 1,
                default: frappe.datetime.now_time()
            },
            { fieldtype: 'Column Break' },
            {
                label: 'Scheduled End Date', fieldname: 'scheduled_end_date', fieldtype: 'Date',
                default: frappe.datetime.get_today()
            },
            {
                label: 'Scheduled End Time', fieldname: 'scheduled_end_time', fieldtype: 'Time'
            },
            { fieldtype: 'Section Break', label: 'Resource Allocation' },
            {
                label: 'Assign Truck', fieldname: 'vehicle', fieldtype: 'Link', options: 'Vehicle', reqd: 1
            },
            {
                label: 'Primary Driver', fieldname: 'driver', fieldtype: 'Link', options: 'Employee', reqd: 1
            },
            { fieldtype: 'Section Break', label: 'Additional Team' },
            {
                label: 'Additional Support Team', fieldname: 'team_members', fieldtype: 'Table MultiSelect', options: 'Enviro Job Team Member', description: 'Search and link additional staff to ride alongside the driver.'
            }
        ],
        size: 'large',
        ...
        primary_action(values) {
            frappe.call({
                ...
                callback: function(r) {
                    if(!r.exc) {
                        frappe.show_alert({message: "Job Scheduled and pushed to Matrix!", indicator: 'green'});
                        d.hide();
