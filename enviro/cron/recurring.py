...
def execute_daily_operations():
    ...
    master_jobs = frappe.get_all("Enviro Job Card", 
        filters={"is_reoccurring_quote": "YES"}, 
        fields=["name", "source_quotation", "type_of_reoccurring", "frequency_in_weeks", "job_creation_date", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"])
    ...
    for job in master_jobs:
        try:
            ...
            frappe.log_error(f"Error processing master job {job.name}: {str(e)}", "Enviro Cron Error")
...
def duplicate_and_schedule(master_job):
    ...
    if new_quote.custom_requires_client_approval == 1:
        try:
            ...
            frappe.log_error(f"Failed to auto-dispatch clone email: {str(e)}")
